...
const RegisterTeam = () => {
  ...
  const totalAmount = selectedPayment === 'IntialDeposit'
  ...
  useEffect(() => {
    ...
    const fetchDivisionDetails = async () => {
      ...
      try {
        const res = await axios.get(`${Url}api/divisions/search?divisionName=${DivisionValue}&page=0&size=10`, {
          headers: {
      ...
      <div className="container mt-5">
        <Row className="justify-content-center">
          <Col className="d-flex flex-column " lg={8}>
            <ContactInformation selectedPayment={selectedPayment} setSelectedPayment={setSelectedPayment} DivisionValue={DivisionValue} setDivisionValue={setDivisionValue} DivisionDetailsBySearch={DivisionDetailsBySearch} totalAmount={totalAmount} />
          </Col>
          <Col className="mt-4">
            {DivisionValue && DivisionDetailsBySearch.length > 0 && <OrderSummeryCard selectedPayment={selectedPayment} DivisionDetailsBySearch={DivisionDetailsBySearch} totalAmount={totalAmount} />}
