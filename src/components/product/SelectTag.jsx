...
const SelectTag = ({ label, error, touched, options, deFaultValue, onChange, className, icon, ...props }) => {
    ...
    return (
      ...
      {label && <label htmlFor="">{label}</label>}
            <div className={` ${icon  && 'd-flex align-items-center' }`}>
                {icon}
                <select {...props} onChange={onChange} className={` ${className}   bg-white`}>
                {deFaultValue &&   <option value="" className="bg-dark text-white">{deFaultValue}</option>}
                    {options?.map(option => (
