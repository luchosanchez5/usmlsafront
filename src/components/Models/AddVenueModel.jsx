...
import { AddCoManagerToTeam, AddVenue, GetCoManager } from '../../store/team/actions/actionsCreators';
        ...
        <Modal show={show} onHide={onClose} size="xl" centered className="py-4">
            ...
            <Modal.Body className="row      justify-content-center">
                <Row className="d-flex flex-column align-items-center">
                    {VenueData?.data?.length > 0 ? VenueData?.data?.map((card, index) => (
                        <Col key={card.id} md={12} className="my-2 " >
                            <Card
                                className={`px-3  ${selectedCard === card.venueId
                                    ? "bg-danger text-white" : "text-black"}`}
                ...
                {VenueData?.data?.length > 10 && <PaginationControl
                    page={page}
                    between={3}
                    limit={10}
